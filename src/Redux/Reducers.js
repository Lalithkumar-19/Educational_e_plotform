...
const updateCart = async (cart, id) => {
    try {
        ...
        if (isInCart() === false) {
            const res = await axios.put("http://localhost:5000/addtocart?token=" + localStorage.getItem("token"), { id: id, quantity: 0 });
            if (res.status === 200) {
...
const addToWishList = async (list, id) => {
    try {
        ...
        if (isInlist() === false) {
            const res = await axios.put("http://localhost:5000/addtowishlist?token=" + localStorage.getItem("token"), { id: id, quantity: 0 });
            if (res.status === 200) {
...
const Remove_From_Cart = async (cart, id) => {
    ...
    try {
        ...
        if (isInCart() === true) {
            const res = await axios.put("http://localhost:5000/Remove_cart_item?token=" + localStorage.getItem("token"), { id: id, quantity: 0 });
            if (res.status === 200) {
                GetUser_details();
            } else {
                console.log("Internal server is error, try again");
...
const Remove_From_WishList = async (list, id) => {
    try {
        ...
        if (isInlist() === true) {
            const res = await axios.put("http://localhost:5000/Remove_widhlist_item?token=" + localStorage.getItem("token"), { id: id });
            if (res.status === 200) {
