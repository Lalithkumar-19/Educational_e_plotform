...
const BlogEditor = () => {
    ...
    const handle_submit = async () => {
        if (title !== "" && content !== "" && image !== null && tags.length > 0 && categories.length > 0 && localStorage.getItem("token") !== null && localStorage.getItem("userdata") !== null) {
            try {
                ...
                const response = await axios.post("http://localhost:5000/postblog?token=" + localStorage.getItem("token"), formdata)
    ...
    return (
        <div className='Blog_writer'>
            ...
            <button type="button" className='submit_button' onClick={handle_submit}>Upload</button>
            {/* </div> */}
