...
const Profilepage = () => {
    ...
    const fetchUserData = async () => {
        try {
            const response = await axios.get(`http://localhost:5000/user_profile?token=${localStorage.getItem("token")}`);
    ...
    const handle_update_dp = async () => {
        try {
            ...
            const response = await axios.put("http://localhost:5000/update_profile_pic?token=" + localStorage.getItem("token"), formdata);
    ...
    const handleUpdateUser = async () => {
        try {
            if (user.email !== "" && user.name !== "" && user.Addresses !== "" && user.profession !== "") {
                const response = await axios.put("http://localhost:5000/update_profile?token=" + localStorage.getItem("token"), user);
                if (dp !== "") {
                    handle_update_dp();
                }
                else {
    ...
    return (
        <div className="user_profile">
            ...
            <div className="user_details">
                {user ? (
                    <>
                        <img className="user_dp" src={"http://localhost:5000/" + user.dp} alt="profile_pic" />
                        {
                            isEditing ? (
