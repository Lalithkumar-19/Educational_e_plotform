...
function FirstStep({ completed }) {
    ...
    const [base64_thumbnail, setBase64_thumbnail] = useState("");
    ...
    useEffect(() => {
        async function fetchdata() {
            ...
            if (localStorage.getItem('course_id')) {
                ...
                // course_id(courseId);
                await fetch(`http://localhost:5000/get_course_details?id=${courseId}`).then((data) => {
                    if (data.status === 200) {
                        data.json().then(d => {
    ...
    const handleCourseTags = () => {
        if (tags.length !== '' || 0) {
            dispatch({ type: 'ADD_TAG', value: Inputgain });
            setInputgain('');
        }
    }
    ...
    const handleCreate_course = async () => {
        ...
        const isInitialStateFilled = () => {
            ...
            return true; // All non-excluded properties have values
        };
        ...
        if (isInitialStateFilled()) {
            ...
            const requestBody = JSON.stringify({ ...state, course_thumbnail: base64_thumbnail });
            let loading = toast.loading("creating the course");
            // Send the data to the API using the fetch function
            const update_url = "http://localhost:5000/update-course?id=" + localStorage.getItem("course_id");
            const posting_url = `http://localhost:5000/create-course?token=${localStorage.getItem("instructor-token")}`;
            await axios.post(saved ? update_url : posting_url, requestBody, {
                ...
                if (response.status === 200) {
                    response.json().then((data) => {
                        ...
                        completed(true);
                        localStorage.setItem("course_id", data && data.data);
                        setSending(false);
